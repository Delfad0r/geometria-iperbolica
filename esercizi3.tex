...
\newcommand{\exerciseIntersectLines}[2]{
\fill[white,name intersections={of={#1} and {#2},total=\t}] {\ifnum \t>0 \foreach \k in {1,...,\t} {(intersection-\k) circle (.75pt) node[black,cross out,draw,scale=.5,very thick] {}}\fi};
}
...
\foreach \h/\dir/\del [count=\i] in {-.5/0/1,.1/180/-1,.6/0/1} {
    ...
    \foreach \de [count=\j] in {0,-2} {
        \draw[name path global=gamma\i\j,red,thick,domain=-2:2,samples=100,postaction={decorate},decoration={markings,mark=between positions 0 and 1 step .5cm with {\arrow[rotate=\dir]{stealth}}}] plot({\x},{.1*\del+\de+\h+f(\x)});
    }
}
};
\begin{scope}[style={opacity=0.3}]
...
\foreach \i in {1,2,3} \foreach \ii in {1,2,3} \foreach \j in {1,2} {
    ...
    \exerciseIntersectLines{alpha\i}{gamma\ii\j}
}
\end{scope}
...
È immediato verificare che $\alpha$ e $\gamma$ si intersecano esattamente in $i(a,b)^2$ punti. Mostriamo dunque che $\alpha$ e $\gamma$ sono in posizione minimale, ossia che non formano bigoni. Se $i(a,b)=1$ non c'è nulla da dimostrare ($\alpha$ e $\gamma$ si intersecano solo in un punto, dunque hanno intersezione algebrica dispari), quindi supponiamo $i(a,b)\ge 2$. Supponiamo per assurdo che esista un bigono $D$, e siano $\hat\alpha$, $\hat\gamma$ i lati di $D$ che giacciono rispettivamente su $\alpha$ e $\gamma$. Distinguiamo alcuni casi.
\begin{itemize}
...
\exerciseIntersectLines{alpha}{gamma};
